...
@interface StoreVRListViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UserModule_ChannelListProtocol,UserModule_ChannelDetailProtocol,UserModule_StoreListProtocol>
...
@implementation StoreVRListViewController
...
- (UIView *)topView
{
    ...
    self.chooceStoreView.searchFoodBlock = ^(NSDictionary *info) {
        NSLog(@"搜索");
        XishaSearchStoreVRViewController * vc = [[XishaSearchStoreVRViewController alloc]init];
...
{
    [SVProgressHUD show];
//    [[UserManager sharedManager] didRequestStoreVRListWithInfo:@{@"command":@30,@"channel_name":@"shop",@"category_id":@0,@"page_size":@(self.page_size),@"page_index":@(self.page_index),@"key":@"",@"sort":@(self.sort),@"is_red":@0} withNotifiedObject:self];
