//
//  TestReportListViewController.m
//  lanhailieren
//
...
@interface TestReportListViewController ()<UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UserModule_ChannelListProtocol,UserModule_ChannelDetailProtocol>
...
@implementation TestReportListViewController
...
- (void)refreshUI_iPhone
{
    ...
    self.chooceStoreView.searchFoodBlock = ^(NSDictionary *info) {
        XishaSearchTestReportViewController * vc = [[XishaSearchTestReportViewController alloc]init];
    ...
    self.collectionview = [[UICollectionView alloc]initWithFrame:CGRectMake(10, 34, kScreenWidth - 20, kScreenHeight - kNavigationBarHeight - kStatusBarHeight - 34) collectionViewLayout:layout];
...
- (void)doResetQuestionRequest
{
    self.page_index = 1;
    [SVProgressHUD show];
    [[UserManager sharedManager] didRequestStoreTestReportListWithInfo:@{@"command":@30,@"channel_name":@"report",@"category_id":@0,@"page_size":@(self.page_size),@"page_index":@(self.page_index),@"key":@"",@"sort":@(0),@"is_red":@0} withNotifiedObject:self];
