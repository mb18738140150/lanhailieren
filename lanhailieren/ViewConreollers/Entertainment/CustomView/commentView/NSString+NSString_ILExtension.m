...
- (NSMutableArray *)itemsForPattern:(NSString *)pattern captureGroupIndex:(NSUInteger)index
{
    ...
    else
    {
        ...
        [regx enumerateMatchesInString:self options:NSMatchingWithTransparentBounds range:searchRange usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
            
            NSRange groupRange = result.range; //[result rangeAtIndex:index];
