...
- (void)setOldString:(NSString *)oldString andNewString:(NSString *)newString{
    NSString *customEmojiPlistName = @"emotionToText.plist";
    ...
    NSString *imgName = dic[@"[100]"];
    NSString *resourcePath = [[NSBundle mainBundle] resourcePath];
    NSString *bundlePath = [resourcePath stringByAppendingPathComponent:@"Emotion.bundle"];
    NSString *image_path = [bundlePath stringByAppendingPathComponent:imgName];
...
- (NSAttributedString *)createAttributedEmotionStringWithRanges:(NSArray *)ranges
    ...
    if (_textColor == nil) {
        _textColor = [UIColor colorWithRed:0.965 green:0.388 blue:0.122 alpha:1];
    }
...
void Draw_Emoji_For_Line(CGContextRef context, CTLineRef line, id owner, CGPoint lineOrigin)
{
    ...
    for(NSInteger i = 0; i < count; i++){
        ...
        if (emojiName){
            ...
            NSString *emojiFilePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:customEmojiPlistName];
            NSDictionary *dic = [[NSDictionary alloc] initWithContentsOfFile:emojiFilePath];
            NSString *imgName = dic[emojiName];
            NSString *resourcePath = [[NSBundle mainBundle] resourcePath];
            NSString *bundlePath = [resourcePath stringByAppendingPathComponent:@"Emotion.bundle"];
            NSString *image_path = [bundlePath stringByAppendingPathComponent:imgName];
...
- (void)manageGesture:(UIGestureRecognizer *)gesture gestureType:(GestureType)gestureType{
    ...
    DELAYEXECUTE(0.3, [self->_selectionsViews makeObjectsPerformSelector:@selector(removeFromSuperview)]);
...
- (BOOL)judgeIndexInSelectedRange:(CFIndex) index withWorkLine:(CTLineRef)workctLine{
    ...
    for (int i = 0; i < _attributedData.count; i ++) {
        ...
        if (index>=keyRange.location && index<= keyRange.location + keyRange.length) {
            ...
            NSLog(@"%@", feedString);
//            for (RCUserInfo * userinfo in self.datauserInfoArr) {
//                NSLog(@"***%@", userinfo.name);
//            }
            
            if (self.datauserInfoArr.count > 0) {
//                RCUserInfo * userInfo = [RCUserInfo new];
//                for (int i = 0; i < self.datauserInfoArr.count; i++) {
//                    RCUserInfo * user = [self.datauserInfoArr objectAtIndex:i];
//                    if ([user.name isEqualToString:feedString]) {
//                        userInfo = user;
//                        break;
//                    }
//                }
//                if (userInfo.userId) {
//                    [_delegate clickWFCoretext:userInfo.userId replyIndex:_replyIndex];
//                }else
//                {
