...
@interface ClubContestViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UserModule_ChannelListProtocol,UserModule_ChannelDetailProtocol>
...
@implementation ClubContestViewController
...
{
    self.navigationItem.title = @"比赛专区";
...
- (void)refreshUI_iPhone
{
    ...
    self.collectionview = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 70, kScreenWidth , kScreenHeight - kNavigationBarHeight - kStatusBarHeight - 70) collectionViewLayout:layout];
...
- (UIView *)topView
{
    ...
    [self.chooceStoreView resetContent:@"搜索您想查看的比赛"];
    __weak typeof(self)weakSelf = self;
    
    self.chooceStoreView.searchFoodBlock = ^(NSDictionary *info) {
        NSLog(@"搜索");
        ClubSearchContestViewController * vc = [[ClubSearchContestViewController alloc]init];
    ...
    [self.saleCountBtn setTitle:@"最新发布" forState:UIControlStateNormal];
...
{
    [SVProgressHUD show];
    [[UserManager sharedManager] didRequestClubContextListWithInfo:@{@"command":@30,@"channel_name":@"game",@"category_id":@0,@"page_size":@(self.page_size),@"page_index":@(self.page_index),@"key":@"",@"sort":@(self.sort),@"is_red":@0} withNotifiedObject:self];
}

- (void)loadData
{
    [SVProgressHUD show];
    [[UserManager sharedManager] didRequestClubContextListWithInfo:@{@"command":@30,@"channel_name":@"game",@"category_id":@0,@"page_size":@(self.page_size),@"page_index":@(self.page_index),@"key":@"",@"sort":@"0",@"is_red":@0} withNotifiedObject:self];
}
    ...
    switch (self.sort) {
        case 0:
        {
            ...
            for (NSDictionary * info in [[UserManager sharedManager] getclubContextList]) {
                ...
                [mInfo setObject:[info objectForKey:@"sub_title"] forKey:@"content"];
