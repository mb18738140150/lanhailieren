...
- (void)layoutSubviews
{
    ...
    if(kTabBarHeight > 49)
    {
        self.topBar.frame = CGRectMake(CGRectGetMinX(self.bounds), CGRectGetMinY(self.bounds), CGRectGetWidth(self.bounds), kTabBarHeight);
    }
    
    // 返回按钮
    self.backButton.frame = CGRectMake(CGRectGetMinX(self.topBar.bounds), CGRectGetHeight(self.topBar.bounds) - 50, 40, 40);
    // 锁定按钮
